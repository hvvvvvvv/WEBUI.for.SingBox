...
const requestWithProgress = () => {
  return async (
    ...
    const {
      ...
      '/net/download', method, url, path, _headers, progressEvent, _options,
    )
...
const requestWithoutBody = (method: RequestMethod.Get) => {
  return async <T = any>(
    url: string,
    headers: Request['headers'] = {},
    options: Request['options'] = {},
    ...
    const {
      ...
      '/net/requests', method, url, _headers, '', _options,
    )
...
export const Download = requestWithProgress()
