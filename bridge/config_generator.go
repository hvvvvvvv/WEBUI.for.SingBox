...
func newConfigGenerator(app *App) (*configGenerator, error) {
...
func (g *configGenerator) loadSubscriptions() error {
	...
	if err := readYAMLFile(subscribesFilePath, &records); err != nil {
...
func (g *configGenerator) loadRulesets() error {
	...
	if err := readYAMLFile(rulesetsFilePath, &records); err != nil {
...
func readYAMLFile(path string, target any) error {
	bytes, err := os.ReadFile(GetPath(path))
	if err != nil {
...
func (g *configGenerator) subscriptionEntries(id string) ([]map[string]any, error) {
	...
	bytes, err := os.ReadFile(GetPath(meta.Path))
	if err != nil {
