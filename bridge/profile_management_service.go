...
func (s *profileManagementService) ListProfiles(
	_ context.Context,
	_ *connect.Request[configv1.ListProfilesRequest],
) (*connect.Response[configv1.ListProfilesResponse], error) {
	s.mu.Lock()
	defer s.mu.Unlock()
	...
	return connect.NewResponse(&configv1.ListProfilesResponse{Profiles: cloneProfiles(profiles)}), nil
}

func (s *profileManagementService) GetProfile(
	_ context.Context,
	req *connect.Request[configv1.GetProfileRequest],
) (*connect.Response[configv1.GetProfileResponse], error) {
	if req.Msg.GetId() == "" {
	...
	for _, profile := range profiles {
		if profile != nil && profile.GetId() == req.Msg.GetId() {
			return connect.NewResponse(&configv1.GetProfileResponse{Profile: cloneProfile(profile)}), nil
		}
	}
...
func (s *profileManagementService) CreateProfile(
	_ context.Context,
	req *connect.Request[configv1.CreateProfileRequest],
) (*connect.Response[configv1.CreateProfileResponse], error) {
...
func (s *profileManagementService) UpdateProfile(
	_ context.Context,
	req *connect.Request[configv1.UpdateProfileRequest],
) (*connect.Response[configv1.UpdateProfileResponse], error) {
	profile := req.Msg.GetProfile()
	if profile == nil || profile.GetId() == "" {
		return nil, asConnectError(invalidArgumentError{message: "profile.id is required"})
	}
	...
	Hub.Emit("profileChange", map[string]any{"id": profile.GetId()})
	return connect.NewResponse(&configv1.UpdateProfileResponse{Profile: cloneProfile(profile)}), nil
}

func (s *profileManagementService) DeleteProfile(
	_ context.Context,
	req *connect.Request[configv1.DeleteProfileRequest],
) (*connect.Response[configv1.DeleteProfileResponse], error) {
	id := req.Msg.GetId()
	if id == "" {
		return nil, asConnectError(invalidArgumentError{message: "id is required"})
	}
	...
	removed := false
	remaining := make([]*configv1.Profile, 0, len(profiles))
	for _, profile := range profiles {
	...
	Hub.Emit("profileChange", map[string]any{"id": id})
...
func (s *profileManagementService) saveProfiles(profiles []*configv1.Profile) error {
	fullPath := GetPath(profilesFilePath)
	if err := os.MkdirAll(GetPath("data"), os.ModePerm); err != nil {
...
func normalizeProfilePayloadsForYAML(profiles []*configv1.Profile) {
	for _, profile := range profiles {
		if profile == nil {
			continue
		}
...
func cloneProfiles(input []*configv1.Profile) []*configv1.Profile {
	output := make([]*configv1.Profile, 0, len(input))
...
func cloneProfile(profile *configv1.Profile) *configv1.Profile {
	if profile == nil {
		return nil
	}
	return proto.Clone(profile).(*configv1.Profile)
}

func migrateLegacyProfilesYAML(raw []byte) ([]*configv1.Profile, error) {
	var entries []map[string]any
	if err := yaml.Unmarshal(raw, &entries); err != nil {
		return nil, fmt.Errorf("parse legacy yaml: %w", err)
	}

	result := make([]*configv1.Profile, 0, len(entries))
